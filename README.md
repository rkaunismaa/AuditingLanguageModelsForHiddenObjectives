...
| SFT stage | Included (paper finds it's a useful but non-essential initialization) | Skipped — goes mid-training → DPO directly, per the reference replication's finding that SFT not only isn't necessary but *hurts* held-out generalization here (see below) |
...
8B model, the subsampled midtrain corpus, and single-epoch DPO. The natural next
levers for a more uniform generalization profile are a larger midtrain subsample and
more DPO epochs — not an SFT stage, which the reference replication's Appendix C
found *lowers* held-out test-bias exploitation (see
[What this repo replicates](#what-this-repo-replicates)).
...
2. **Try to fix the unevenness.** The design spec already flagged "8B may generalize
   ...
   (75k → more of the full 523k) and add DPO epochs. (Adding an SFT stage is *not* a
   candidate here — the reference replication's Appendix C found it lowers held-out
   generalization; see [What this repo replicates](#what-this-repo-replicates).)
